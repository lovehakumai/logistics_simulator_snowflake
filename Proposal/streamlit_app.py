...
with st.sidebar.form("Parameter"):
    ...
    port_list = ["USA", "Japan", "Philippines"]
    with col1: 
        origin_port = st.selectbox("Origin", port_list)
    with col2:
        destination_port = st.selectbox("Destination", port_list)
    

    weather_list = ["All", "Rain","Fog","Clear", "Hurricane", "Storm"]
    weather_param = st.selectbox("Weather", weather_list)
    transport_mode_list = ["Airplane", "Ship", "Rail"]
    transport_param = st.selectbox("Transport", transport_mode_list)
    carrier_reliability_param = st.slider("Carrier reliability", 0.0, 1.0, 0.1)
    fuel_increase_param = st.slider("Fuel Cost Increasing Factor", 0.0, 1.0, 0.1)
    risk_avoidance_param = st.slider("Risk Avoidance Factor", 0.0, 1.0, 0.1)
