...
def create_table_from_zip(session, zip_full_path, target_db, target_schema):
    file_stream = session.file.get_stream(zip_full_path, decompress=False) # decompress is used when the file is single compressed files like .gzip
    file_cnt = 1
    with ZipFile(file_stream) as zip:
        ...
        for nm in target_name_list:
            ...
            session.sql(f"USE DATABASE {source_db_name}").collect()
...
zip_path = ['@"KAGGLE_LOGISTICS"."RAW"."GLOBAL_SUPPLY_CHAIN_RISK_AND_LOGISTICS_2024_2026"/logistics.zip']

for i in range(len(zip_path)):
    create_table_from_zip(session, zip_full_path=zip_path[i], target_db='KAGGLE_LOGISTICS', target_schema='RAW')
